                        ...
                        for ns in i.items:
                            ...
                            if (ans_A.flags & 0x0F == 0):
                                for j in ans_A.answer:
                                    for ip in j.items:
                                        if ip.rdtype==1:
                                            with open(filename.strip(".txt") + '_domain_ns_a_edu.txt', 'a') as f:
                                                f.write(domain + "\t" + str(ns).strip(".") + "\t" + str(ip) + "\n")
                            else:
                                with open(filename.strip(".txt") + '_domain_ns_a_edu.txt', 'a') as f:
                                    f.write(domain + "\t" + str(ns).strip(".") + "\t" +flag_dict[ans_A.flags & 0x0F] + "\n")
            else:
                with open(filename.strip(".txt") + '_domain_ns_a_edu.txt', 'a') as f:
                    f.write(domain + "\t" + flag_dict[ans.flags & 0x0F] + "\t" + flag_dict[ans.flags & 0x0F] + "\n")
